...
class Game(object):
    ...
    def __init__(self, window):
        ...
        self.chest_pos = {}
        self.window = window
        self.change_map(os.path.join('Maps','1.tmx'), self.player.x, self.player.y)
    ...
    def update(self):
        ...
        for direction in dirs:
            if (self.player.x + direction[0], self.player.y + direction[1]) in self.map.Signs:
                sign = self.map.Signs[(self.player.x + direction[0],
                        self.player.y + direction[1])]
                self.draw_message(sign.properties['Message'], sign.x,
                                  sign.y)
            if (self.player.x + direction[0], self.player.y + direction[1]) in self.chest_pos:
                chest = self.chest_pos[(self.player.x + direction[0],
                        self.player.y + direction[1])]
                if chest.map == self.map_file:
                    self.draw_message('Contains: {0} Gold. E>Give Q>Take'.format(str(chest.gold)),
                            chest.rect.x, chest.rect.y)
        gold_pieces = pygame.sprite.spritecollide(self.player,
                self.gold_pieces, True)
    ...
    def down(self):
        ...
        if (self.player.x, self.player.y + 1) in self.map.Land:
            ...
            self.player.down()
            self.change_map(obj.properties['Connected'],
                            obj.properties['GoX'], obj.properties['GoY'
                            ])
    ...
    def take(self):
        dirs = [(1, 0), (-1, 0), (0, 1), (0, -1)]
        for direction in dirs:
            if (self.player.x + direction[0], self.player.y
                + direction[1]) in self.chest_pos:
                chest = self.chest_pos[(self.player.x + direction[0],
                        self.player.y + direction[1])]
                if chest.map == self.map_file:
    ...
    def change_map(self,map_file,x,y):
        ...
        self.player.x = x
        self.player.y = y
        self.window.center(x * 16, y * 16, self.map.Map.width * 16,
                           self.map.Map.height * 16)
        ...
        for (pos, obj) in self.map.Gold.items():
            if (pos[0], pos[1]) not in self.gold_taken[self.map_file]:
                self.gold_pieces.add(Gold(pos[0], pos[1], self.window,
                        obj.image, obj.properties['Worth']))
        for (pos, image) in self.map.Chests.items():
            if (pos[0], pos[1]) not in self.chest_pos:
                chest = Chest(pos[0], pos[1], self.window, image,
                              self.map_file)
                self.chests.add(chest)
                self.chest_pos[pos[0], pos[1]] = chest
        if self.map.Map.width > 30:
            for i in range(random.randint(settings.min_wolves,
                           settings.max_wolves)):
                pos = random.choice(self.map.Land)
                self.wolves.add(Wolf(pos[0], pos[1], self.window,
                                self.map.Land, self.player))
...
class Map(object):
    ...
    def make_map(self, map_file):
        ...
        self.image = pygame.Surface([self.Map.width * 16,
                                    self.Map.height * 16]).convert()
        self.overlay = pygame.Surface([self.Map.width * 16,
                self.Map.height * 16], pygame.SRCALPHA,
                32).convert_alpha()
...
class App(object):
    ...
    def main_loop(self):
        ...
        while not self.done:
            ...
            self.screen.blit(self.game.map.image, (self.window.x,
                             self.window.y))
            self.sprites.draw(self.screen)
            self.screen.blit(self.game.map.overlay, (self.window.x,
                             self.window.y))
            self.top_sprites.draw(self.screen)
            gold = utils.draw_font('Gold: ' + str(self.game.gold), 24,
                                   settings.FONT)
            self.screen.blit(gold, (self.screen.get_width()
                             - gold.get_width(),
                             self.screen.get_height()
                             - gold.get_height()))
            all_gold = utils.draw_font('All Gold: '
                    + str(self.game.all_gold), 24, settings.FONT)
            self.screen.blit(all_gold, (self.screen.get_width()
                             - all_gold.get_width(),
                             self.screen.get_height()
                             - all_gold.get_height()
                             - gold.get_height() - 4))
...
class MainMenu(object):
    ...
    def make_gui(self):
        ...
        b2 = Button('Quit', self.screen.get_width() / 2, b1.rect.bottom
                    + 10)
        b2.rect.x -= b2.rect.width / 2
        self.buttons.add(b1, b2)
        self.actions[b1] = [self.quit]
        self.actions[b2] = [sys.exit]
...
def main():
    ...
    pygame.display.set_mode((infoObject.current_w,
                            infoObject.current_h), pygame.FULLSCREEN)  # Prepare the screen.
    pygame.display.set_icon(pygame.transform.scale(pygame.image.load(settings.ICON),
                            (64, 64)))
